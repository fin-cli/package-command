...
use FIN_CLI\Loggers\Execution;
use FIN_CLI\Tests\TestCase;

require_once VENDOR_DIR . '/fin-cli/fin-cli/php/utils.php';
require_once VENDOR_DIR . '/fin-cli/fin-cli/php/class-fin-cli.php';
require_once VENDOR_DIR . '/fin-cli/fin-cli/php/class-fin-cli-command.php';
...
class ComposerJsonTest extends TestCase {
	...
	public function set_up() {
		...
		// Save and set logger.
		$class_fin_cli_logger = new \ReflectionProperty( 'FIN_CLI', 'logger' );
		if ( PHP_VERSION_ID < 80100 ) {
			$class_fin_cli_logger->setAccessible( true );
		}
		$this->prev_logger = $class_fin_cli_logger->getValue();

		$this->logger = new Execution();
		FIN_CLI::set_logger( $this->logger );
		...
		$this->temp_dir = Utils\get_temp_dir() . uniqid( 'fin-cli-test-package-composer-json-', true ) . '/';
		mkdir( $this->temp_dir );
	}

	public function tear_down() {
		// Restore logger.
		FIN_CLI::set_logger( $this->prev_logger );
		...
		$class_fin_cli_capture_exit->setValue( null, $this->prev_capture_exit );
	...
	public function test_create_default_composer_json() {
		...
		$this->assertTrue( false !== strpos( file_get_contents( $actual ), 'fin-cli/fin-cli' ) );
		unlink( $actual );
		rmdir( dirname( $actual ) );
	}

	public function test_get_composer_json_path() {
		$env_test                = getenv( 'FIN_CLI_TEST_PACKAGE_GET_COMPOSER_JSON_PATH' );
		$env_home                = getenv( 'HOME' );
		$env_fin_cli_packages_dir = getenv( 'FIN_CLI_PACKAGES_DIR' );
		...
		putenv( 'FIN_CLI_TEST_PACKAGE_GET_COMPOSER_JSON_PATH=1' );
		putenv( 'HOME=' . $this->temp_dir );

		// Create in HOME.
		putenv( 'FIN_CLI_PACKAGES_DIR' );
		$expected = $this->temp_dir . '.fin-cli/packages/composer.json';
		...
		// Create in FIN_CLI_PACKAGES_DIR.
		putenv( 'FIN_CLI_PACKAGES_DIR=' . $this->temp_dir . 'packages' );
		$expected = $this->temp_dir . 'packages/composer.json';
		$actual   = $get_composer_json_path->invoke( $package );
		$this->assertSame( $expected, $this->mac_safe_path( $actual ) );
		$this->assertTrue( false !== strpos( file_get_contents( $actual ), 'fin-cli/fin-cli' ) );
		...
		putenv( false === $env_test ? 'FIN_CLI_TEST_PACKAGE_GET_COMPOSER_JSON_PATH' : "FIN_CLI_TEST_PACKAGE_GET_COMPOSER_JSON_PATH=$env_test" );
		putenv( false === $env_home ? 'HOME' : "HOME=$env_home" );
		putenv( false === $env_fin_cli_packages_dir ? 'FIN_CLI_PACKAGES_DIR' : "FIN_CLI_PACKAGES_DIR=$env_fin_cli_packages_dir" );
	}
